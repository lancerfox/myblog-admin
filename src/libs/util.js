...
/**
 * @description    把json转换为一维数组
 * @param     {Object} 要转换的json
 * @return    { Array }
 */
export const jsonToArray = function (nodes) {
  let r = []
  if (Array.isArray(nodes)) {
    nodes.forEach(item => {
      ...
      if (Array.isArray(item['children']) && item['children'].length > 0) {
        ...
        r = r.concat(jsonToArray(item['children']))
...
/**
     * @description  把一维数组转变为树状结构数组
     * @param     {Array}  data 要转换的一维数组
     * @return    {Array}  树状结构数组
     */
export const changeToTreeData = function (data) {
